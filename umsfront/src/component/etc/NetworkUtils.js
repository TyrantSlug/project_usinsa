...
export function InsertFetchFn(servicename, dto, is) {
  // *** (x)
  return fetchFn("POST", `/api/${servicename}/${is}`, dto).then((data) => {
    if (data === undefined) {
      return;
    }
...
export function UpdateFetchFn2(servicename, dto) {
  // *** (x)
  return fetchFn("PUT", `/api/${servicename}`, dto).then((data) => {
    if (data === undefined) {
      return;
    }
    if (servicename === "member-service") {
...
export function UpdateFetchFn(servicename, dto, is) {
  // *** (x)
  return fetchFn("PUT", `/api/${servicename}/${is}`, dto).then((data) => {
    if (data === undefined) {
      return;
    }
    if (servicename === "member-service") {
...
export function DeleteFetchFn(servicename, dto, is) {
  /// *** (x)
  return fetchFn("DELETE", `/api/${servicename}/${is}`, dto).then((data) => {
    if (data === undefined) {
      return;
    }
